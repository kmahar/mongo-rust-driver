...
use crate::{event::sdam::{
    ...
    TopologyOpeningEvent, TopologyDescription,
...
use super::{
    ...
    TOPOLOGY_TRACING_EVENT_TARGET, DEFAULT_MAX_DOCUMENT_LENGTH_BYTES,
};
...
impl SdamEventHandler for TopologyTracingEventEmitter {
    ...
    fn handle_server_opening_event(&self, event: ServerOpeningEvent) {
        if trace_or_log_enabled!(target: TOPOLOGY_TRACING_EVENT_TARGET, TracingOrLogLevel::Debug) {
            tracing::debug!(
